...
The full API surface as of the original submission is documented in two companion artifacts included alongside this repo:
- A ready-to-import **Postman collection** covering the original 20 endpoints, with auth tokens and IDs auto-captured between requests via test scripts
- A printable **PDF reference** with sample request/response bodies for every endpoint

Neither artifact has been regenerated yet to include the FPO pooling, planning, and logistics endpoints added afterward (see the table below).
...
| FPO Pools | create pool, list open pools for my FPO, contribute a lot, aggregate, get payout breakdown |
| Planning | pre-harvest demand forecast, ranked crop recommendations |
| Logistics | optimize a pooled pickup route |

The three rows above (FPO Pools, Planning, Logistics) are not yet in the Postman collection or PDF reference — they were added after those artifacts were generated.
...
## Where We Closed the Gap

A head-to-head review against a comparable project (AgriDirect) flagged four real, structural gaps. Three are now closed in this codebase; the fourth is honestly still open.

| Gap flagged | What shipped | Where |
|---|---|---|
| No FPO/bargaining-power aggregation | Farmers in the same FPO pool several already-graded lots into one larger listing. The merge produces an ordinary `CropLot`, so it flows through the *existing* bidding/contract/fulfillment pipeline unchanged — no auction logic was duplicated. Settled price splits back out pro-rata per contributor. | `service/FpoPoolingService`, `controller/FpoPoolingController`, `domain/crop/FpoPooledLot(Contribution)` |
| No pre-harvest demand forecasting ("what to grow") | RAG-grounded forecast reusing the same Qdrant-ingested mandi bulletins the reserve-price advisor already trusts — same discipline: answers "insufficient data" rather than guessing when the retrieved context doesn't support a real trend judgement. Runs before a `CropLot` exists at all. | `ai/planning/DemandForecastService`, `ai/planning/PlanningController` |
| No named, concrete routing engine | Nearest-neighbor construction + 2-opt local-search improvement over real GPS (farmer pickup coordinates already captured on `CropLot`, warehouse coordinates added in V8) using Haversine distance — a genuine, well-studied heuristic, not a capacity lookup. Exposed as both a REST endpoint and a negotiation-copilot tool. | `ai/logistics/RouteOptimizationService`, `ai/tools/RouteOptimizationTool` |
| English-only, not vernacular | Every farmer-facing AI narrative (reserve-price rationale, demand-forecast narrative, crop recommendations) now responds in the farmer's stored `preferredLanguage`, in its native script, in plain vocabulary — not applied to crop-grading output, since that's structured data matched against an English pest-tag table and localizing it would break the matching. | `ai/util/LanguageInstructions`, `domain/user/User.preferredLanguage` |

Two bugs surfaced and were fixed along the way: the reserve-price advisor's RAG query had a hardcoded instruction forcing a "1 rupee" price for any low-grade lot regardless of retrieved context, and registration was silently discarding `fpoAffiliation` before it ever reached the database — both fixed as part of wiring FPO pooling through.

**What's still genuinely open:** the actual client. This closes the *backend* half of the accessibility gap — every AI response can now come back in a farmer's language — but a REST API is still not what a first-time smartphone user in a village opens. The comparison's frontend critique stands until a real (ideally React Native, low-bandwidth, vernacular-first) client exists; see Roadmap.
...
- No frontend yet — every capability described above, including the newer FPO/planning/logistics features, is verified through the API directly
- Automated integration test coverage beyond the one concurrency test is still to be built, and the FPO pooling / demand forecast / route optimization additions have not yet been through a compiled build or integration test pass — they were written against the existing codebase's established patterns but should be treated as needing a build-and-test pass before being trusted in a demo
- The route optimizer's 2-opt heuristic is tuned for small stop counts (a handful to a few dozen pickups per run); a real VRP solver (jsprit, OptaPlanner) would be the next step if route sizes grow past that
- Crop recommendation ranking is capped at the first 15 known categories per call — fine for the current reference-data set, but would need pagination or batching if the category catalog grows much larger

## Roadmap

- Automated integration tests across the full auction-to-contract-to-fulfillment lifecycle, extended to cover FPO pooling and route optimization
- A proper admin-provisioning flow that doesn't require direct database access
- OCR support for scanned document ingestion
- A frontend — vernacular-first, low-bandwidth, React Native, matching the language support the backend already provides
- A real VRP solver for larger pooled-transport routes
