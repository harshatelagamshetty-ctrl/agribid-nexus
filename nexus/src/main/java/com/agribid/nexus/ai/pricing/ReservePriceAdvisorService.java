...
public class ReservePriceAdvisorService {
    ...
    public ReservePriceSuggestion suggestReservePrice(Long lotId) {
        ...
        // NOTE: this previously ended with "...and if the grade of the
        // crop is very low you can just say explicitly that the price
        // is 1 rupee" — a hardcoded instruction baked into the RAG
        // query string itself, not something derived from retrieved
        // context or the lot's actual grade. It silently overrode the
        // "ground your recommendation only in retrieved context" rule
        // below for every low-grade lot. Removed — a low grade should
        // still get a real, retrieval-grounded price, just a lower one.
        ...
        String userMessage = reservePricePromptTemplate.render(Map.of(
                ...
                "languageInstruction", LanguageInstructions.instructionFor(lot.getOwner().getPreferredLanguage())
        ));
